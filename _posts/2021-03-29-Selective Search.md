...
> 每个颜色通道使用25个bin值，输入时彩色图像，有3个通道，所以一共有75个bin值。
>
> 区域$r_i$ 的颜色向量$C_i=\{c_i^1,...,c_i^n\}$，其中，n=75。
>
> 颜色向量使用$L_1$ 范数进行了正则化处理。
...
作者将以上四种方式计算出来的相似度值，加权求和，得到两个区域的最终相似度：
$$
